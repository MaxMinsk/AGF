...
| `M24` Rapier physics & colliders | **Active — adapter + sync + sensors + character shipped** | Source: `Notes/colliders_physics_implementation_analysis.md` (Rapier vs Ammo/Jolt + Three.js examples survey). Verdict: **adopt Rapier** as an *adapter*, not a gameplay API. ECS + JSON stay the source of truth; Rapier handles live only in internal `entityToBody` / `entityToColliders` maps (M22 derived-cache rules apply — rebuildable, parity-tested). Shipped (S35–S36): `M24-investigate` ✅ Rapier bundles via Vite (~1.6–1.8 MB gzipped, lazy-load required), `M24-schema` ✅ `RigidBody3D` + `Collider3D` + `CharacterController3D` JSON schemas with per-kind constraints, `M24-adapter` ✅ lazy-init `engine/physics/rapier/rapier-adapter.ts` + primitive body/collider lifecycle, `M24-sync` ✅ `PhysicsSyncSystem` (5-phase fixedUpdate: acquire/release / kinematic push / step / sensors / dynamic writeback), `M24-sensors` ✅ runtime-only `OverlappingTriggers3D` / `CurrentContacts3D` from Rapier's `EventQueue`, `M24-character` ✅ kinematic capsule controller wrapper. Beacon adopted physics declarations (S36) — drone+ground+sensors registered; gameplay still drives via Transform distance (sensor-wiring + character-movement system are Sprint 37 candidates). Remaining: `M24-raycast`, `M24-debug` (`world.debugRender()` overlay + `engine doctor` body counts + HMR leak test), `M24-static-mesh` (trimesh + heightfield from GLB), `beacon-physics-character`, `beacon-physics-sensor-wiring`. Deferred: joints, vehicles, ragdolls, soft bodies, 2D physics, server-authoritative replication. Multiplayer note: Rapier does NOT guarantee cross-client lockstep — first networked sample stays gameplay-level state, not raw physics. |
