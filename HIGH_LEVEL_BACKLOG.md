...
| `M24` Rapier physics & colliders | **Investigated — staged plan** | Source: `Notes/colliders_physics_implementation_analysis.md` (Rapier vs Ammo/Jolt + Three.js examples survey). Verdict: **adopt Rapier** as an *adapter*, not a gameplay API. ECS + JSON stay the source of truth; Rapier handles live only in internal `entityToBody` / `entityToColliders` maps (M22 derived-cache rules apply — rebuildable, parity-tested). Two-layer collision stack: (1) interaction picking via Three.js raycaster / instanced `instanceId` (`M18`); (2) simulation via Rapier (`M24`). Components: `RigidBody3D` (fixed / dynamic / kinematicPosition), `Collider3D` (box / sphere / capsule / cylinder; later trimesh / heightfield / compound), `PhysicsMaterial3D`, `CharacterController3D`, sensor via `Collider3D.sensor`. Collision output is hybrid: raw events → runtime-only derived components (`Grounded3D`, `OverlappingTriggers3D`, etc., visible to `inspect` / snapshots but never authored) → gameplay components via command buffer. Named collision layers (not bitmasks). Fixed-step pipeline at 1/60 with max-substeps cap. Stories: `M24-investigate` (Rapier WASM bundling spike) → `M24-schema` → `M24-adapter` (primitive bodies + lifecycle) → `M24-sync` (Transform/LocalToWorld ↔ Rapier) → `M24-sensors` (triggers + events + handle-to-entity map) → `M24-raycast` (returns AGF EntityIds, not Rapier objects) → `M24-character` (kinematic capsule controller) → `M24-debug` (Rapier `debugRender()` overlay + `engine doctor` body/collider counts) → `M24-static-mesh` (fixed trimesh + heightfield). Deferred: joints, vehicles, ragdolls, soft bodies, 2D physics, server-authoritative replication. Multiplayer note: Rapier does NOT guarantee cross-client lockstep — first networked sample stays gameplay-level state, not raw physics. First sample shape: player capsule + walking on fixed colliders + pickup/hazard sensors + a couple of dynamic crates. |
