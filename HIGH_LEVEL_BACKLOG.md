...
| `M20` Netcode rework — pick a proven model & rebuild own-drone authority | **High priority — investigate first** | Three bugs surfaced together in Sprint 32 (own-drone runs 2× the server-broadcast position; 30s server idle timeout disconnects steady players; networked controls feel different from single-player). Current "rollback-replay with `replayUnackedIntents`" is mixed-source-of-truth — PlayerInputSystem moves the drone locally AND `network-drone-sync` lerps it toward `server + replay`, double-counting in-flight intents. Needs an investigation pass that picks a proven netcode pattern (client-side prediction + server reconciliation à la Valve, snapshot interpolation only à la Quake 3, deterministic lockstep, rollback netcode like GGPO) and a sequenced rebuild. Investigation lands as a research doc; implementation sprints sized after. See `docs/research/netcode-rework-investigation.md` (to be written). |
