...
Date: 2026-05-14 (Sprint 37 archived)
...
| `M24` Rapier physics & colliders | **Active — adapter + sync + sensors + character + debug + interpolation + Beacon sensor-wiring shipped** | Source: `Notes/colliders_physics_implementation_analysis.md` (Rapier vs Ammo/Jolt + Three.js examples survey). Verdict: **adopt Rapier** as an *adapter*, not a gameplay API. ECS + JSON stay the source of truth; Rapier handles live only in internal `entityToBody` / `entityToColliders` maps (M22 derived-cache rules apply — rebuildable, parity-tested). Shipped (S35–S37): `M24-investigate` / `M24-schema` / `M24-adapter` / `M24-sync` / `M24-sensors` / `M24-character` / `M24-debug` ✅ (Rapier `debugRender()` → LineSegments overlay, `__agf.physics.setDebugOverlay(boolean)` + `?physicsDebug=1`) / `M24-interpolation` ✅ (`physicsAlpha` on TimeContext, prev/curr buffer in `PhysicsSyncSystem`, lerp in frameUpdate — fixes 60 Hz pulse on 120 Hz displays) / `beacon-physics-sensor-wiring` ✅ (pickup-system + hazard-system read `OverlappingTriggers3D`). Remaining: `M24-raycast` (`runtime.physics.raycast({...})` API), `M24-static-mesh` (trimesh + heightfield from GLB), `beacon-physics-character` (switch drone to `CharacterController3D` + new `CharacterMovementSystem`). Deferred: joints, vehicles, ragdolls, soft bodies, 2D physics, server-authoritative replication. Multiplayer note: Rapier does NOT guarantee cross-client lockstep — first networked sample stays gameplay-level state, not raw physics. |
...
| `M25` Production asset pipeline | **Investigated — staged plan** | Source: `Notes/utsubo_threejs_best_practices_100_tips.md` (Three.js best-practices survey from utsubo.com). Plan: per-project `engine asset optimize` CLI built on `gltf-transform` that runs GLB compression (Meshopt or Draco), texture compression (KTX2 / Basis Universal — UASTC for normal maps + hero textures, ETC1S for secondary diffuse), and produces LOD chains where source meshes carry an LOD intent. Decoder paths configured once at adapter init (shared `DRACOLoader` / `KTX2Loader` with `.detectSupport(renderer)`); gameplay code never constructs loaders. Asset metadata records pipeline settings + source ref so the output is reproducible. Stories: `ASSET-decoder-paths` → `ASSET-compression` (KTX2 + Draco/Meshopt loaders wired into `GLTFLoader`) → `ASSET-gltf-transform-investigate` → `ASSET-optimize-command` (`engine asset optimize`) → `ASSET-lod-metadata` (LOD schema + `engine check`) → `ASSET-texture-doctor` (warn on huge uncompressed textures, NPOT mismatches for compression, multiple KTX2 instances, missing transcoder config). Deferred: texture atlases (M17-atlas-investigate), array textures, CDN delivery. |
...
| `M21` Renderer → ECS systems | **Phase 1 ✅, Phase 2 in flight (CSM + frame-timing shipped S37)** | `M21-investigate` shipped: see `docs/research/renderer-ecs-split-investigation.md`. **Phase 1 (structural split, 8 stories)**: thin `ThreeRenderAdapter` + five scheduler-registered systems + five renderer-internal components. Renderer-import-boundary preserved. Stories `M21-a..g` + `M21-boundary-check`. **Phase 2 progress through S37**: `M21-light-*` (all 6 kinds ✅), `M21-shadow-basic` ✅, `M21-shadow-csm` ✅ (cascade shadow maps via `three/addons/csm/CSM.js`, see `examples/shadows-bench`), `M21-env-generated` ✅ (RoomEnvironment via PMREMGenerator), `M21-mat-*` (5 shader kinds: standard/physical/lambert/phong/basic ✅), `M21-frame-timing` ✅ (per-phase tick timings on `__agf.frameTiming()` + dev overlay). Phase 2 remaining: `M21-shadow-static` (autoUpdate=false for declared-static casters), `M21-shadow-algorithm` (PCSS / VSM stretch), `M21-mat-custom` (ShaderMaterial + onBeforeCompile), `M21-mat-textures` (KTX2 path), `M21-color` (output color space + tonemap), `M21-env-hdr` / `M21-env-cube`, `M21-cam-*`, `M21-post-pipeline` (Composer + Bloom / FXAA / SSAO / Outline), `M21-light-budgets`, `M21-context-loss`, `M21-tsl-investigate`, `M21-webgpu-spike`. Out of scope at Phase 2 end: real-time GI, Forward+, animation system, scene editor. |
