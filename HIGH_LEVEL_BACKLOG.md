...
Date: 2026-05-14 (Sprint 38 archived)
...
| `M24` Rapier physics & colliders | **Active — M24-static-mesh is the only remaining adapter story** | Source: `Notes/colliders_physics_implementation_analysis.md` (Rapier vs Ammo/Jolt + Three.js examples survey). Verdict: **adopt Rapier** as an *adapter*, not a gameplay API. Shipped (S35–S38): `M24-investigate` / `M24-schema` / `M24-adapter` / `M24-sync` / `M24-sensors` / `M24-character` / `M24-debug` (LineSegments overlay) / `M24-interpolation` (`physicsAlpha` on TimeContext, prev/curr buffer + lerp) / `M24-raycast` (`__agf.physics.raycast`) / `beacon-physics-sensor-wiring` / `beacon-physics-character` (drone now routed through `CharacterMovementSystem` + Rapier KinematicCharacterController). Remaining: `M24-static-mesh` (trimesh + heightfield from GLB), the only adapter story not yet started. Deferred: joints, vehicles, ragdolls, soft bodies, 2D physics, server-authoritative replication. Multiplayer note: Rapier does NOT guarantee cross-client lockstep — first networked sample stays gameplay-level state, not raw physics. |
...
| `M25` Production asset pipeline | **Active — decoder-paths scaffolding shipped S38** | Source: `Notes/utsubo_threejs_best_practices_100_tips.md` (Three.js best-practices survey from utsubo.com). Plan: per-project `engine asset optimize` CLI built on `gltf-transform` that runs GLB compression (Meshopt or Draco), texture compression (KTX2 / Basis Universal — UASTC for normal maps + hero textures, ETC1S for secondary diffuse), and produces LOD chains where source meshes carry an LOD intent. Shipped: `ASSET-decoder-paths` ✅ (process-wide singletons for DRACOLoader / KTX2Loader / MeshoptDecoder in `engine/render/asset-decoders/decoders.ts`; `createGlbLoader({ draco?, ktx2?, meshopt? })` opt-in). Remaining stories: `ASSET-compression` (flip the flags on for a real project), `ASSET-gltf-transform-investigate`, `ASSET-optimize-command` (`engine asset optimize`), `ASSET-lod-metadata` (LOD schema + `engine check`), `ASSET-texture-doctor`. Deferred: texture atlases, array textures, CDN delivery. |
...
| `M21` Renderer → ECS systems | **Phase 1 ✅, Phase 2 in flight (CSM + frame-timing + shadow-static + light-budgets shipped through S38)** | `M21-investigate` shipped: see `docs/research/renderer-ecs-split-investigation.md`. **Phase 1 (structural split, 8 stories)**: thin `ThreeRenderAdapter` + five scheduler-registered systems + five renderer-internal components. Renderer-import-boundary preserved. Stories `M21-a..g` + `M21-boundary-check`. **Phase 2 progress through S38**: `M21-light-*` (all 6 kinds ✅), `M21-shadow-basic` ✅, `M21-shadow-csm` ✅ (cascade shadow maps via `three/addons/csm/CSM.js`, see `examples/shadows-bench`), `M21-shadow-static` ✅ (`autoUpdate=false` for declared-static casters), `M21-env-generated` ✅, `M21-mat-*` (5 shader kinds ✅), `M21-frame-timing` ✅, `M21-light-budgets` ✅ (lights + shadowCasters tracked by performance-budget). Phase 2 remaining: `M21-shadow-algorithm` (PCSS / VSM stretch), `M21-mat-custom` (ShaderMaterial + onBeforeCompile), `M21-mat-textures` (KTX2 path — pairs with M25 ASSET-compression), `M21-color` (output color space + tonemap), `M21-env-hdr` / `M21-env-cube`, `M21-cam-*`, `M21-post-pipeline` (Composer + Bloom / FXAA / SSAO / Outline), `M21-context-loss`, `M21-tsl-investigate`, `M21-webgpu-spike`. Out of scope at Phase 2 end: real-time GI, Forward+, animation system, scene editor. |
