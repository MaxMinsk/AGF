...
Date: 2026-05-14 (Sprint 42 archived)
...
| `M21` Renderer → ECS systems | **Phase 1 ✅, Phase 2 ~99% shipped through S51** | `M21-investigate` shipped: see `docs/research/renderer-ecs-split-investigation.md`. **Phase 1 (structural split, 8 stories)**: thin `ThreeRenderAdapter` + five scheduler-registered systems + five renderer-internal components. Renderer-import-boundary preserved. **Phase 2 shipped through S51**: `M21-light-*` ✅, `M21-shadow-basic` / `-csm` / `-static` / `-algorithm` (PCF + VSM + PCSS — BASIC variant only) ✅, `M21-shadow-pcss-modern` ✅ (S44), `M21-shadow-pcss-csm` ✅ (S47), `M21-env-generated` / `-hdr` / `-cube` ✅ (S44), `M21-mat-*` (5 builtin shader kinds + textures + custom ShaderMaterial + external shader-file refs) ✅, `M21-frame-timing` ✅, `M21-light-budgets` ✅, `M21-color` ✅, `M21-context-loss` ✅, `M21-post-pipeline` ✅, `M21-cam-orbit` / `-follow` / `-cinematic` ✅ (cinematic S47), `M21-tsl-investigate` ✅ (verdict: defer until WebGPU), `RUNTIME-renderer-ready` ✅. **S51 shadow perf deep-dive** ([`docs/research/m21-shadows-bench-perf.md`](docs/research/m21-shadows-bench-perf.md)): cascade count is the dominant lever (3 → 2 = −17 % renderMs); PCSS only +6.5 %; `programs` flat (no shader churn). Sprint 52 picks up `M21-shadow-static-caster-tag` (tag dynamic vs static casters, restore `autoUpdate=false` for static ones — expected −25 % renderMs). Phase 2 remaining: `M21-webgpu-spike`, `M21-shadow-soft`, `M21-shadow-glb-acne`. Out of scope at Phase 2 end: real-time GI, Forward+, animation system, scene editor. |
