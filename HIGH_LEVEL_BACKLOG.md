...
| `M21` Renderer → ECS systems | **High priority — investigate first** | `ThreeRenderer` is a single class with internal `refreshCamera` / `refreshMeshes` / asset-binding caches. Splitting it into discrete scheduler-registered ECS systems (`CameraSyncSystem`, `MeshLifecycleSystem`, `MeshTransformSyncSystem`, `MaterialBindingSystem`, future `BatchingSystem`) gives a cleaner data flow, makes the renderer agent-inspectable, lets `M17` batching slot in as a sibling system, and removes hidden state. Investigation must (a) audit current renderer responsibilities, (b) propose the system split + per-system in/out components, (c) preserve the existing renderer-import-boundary lock, (d) measure perf impact (the per-frame Map lookups must stay competitive with the current class). Output: `docs/research/renderer-ecs-split-investigation.md`, then sequenced implementation. |
