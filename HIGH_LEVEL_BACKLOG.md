...
| `M21` Renderer → ECS systems | **Investigated — ready to implement** | `M21-investigate` shipped: see `docs/research/renderer-ecs-split-investigation.md`. Splits `ThreeRenderer` (453 lines, 12 responsibilities) into a thin `ThreeRenderAdapter` (GL device + scene-graph root + Mesh handle map) + five scheduler-registered systems: `TransformResolveSystem` → `CameraSyncSystem` → `MeshLifecycleSystem` → `MaterialBindingSystem` → `MeshTransformSyncSystem`. Five new renderer-internal components (`LocalToWorld`, `RenderMeshHandle`, `AppliedGeometryRef`, `AppliedMaterialRef`, `ActiveCamera`). Preserves renderer-import-boundary (systems live under `engine/render/systems/`, not `engine/core/`). Gated on `M22 / ECS-B*` benchmark harness — frame-time at 1k entities ≤ baseline × 1.05 per story. Implementation queue: `M21-a` adapter, `M21-b` transform-resolve, `M21-c` camera-sync, `M21-d` mesh-lifecycle, `M21-e` material-binding, `M21-f` mesh-transform-sync, `M21-g` snapshot internals flag + doctor leak check, `M21-boundary-check` engine-check import rule. |
| `M22` ECS performance & design discipline | **Active** | Source: `Notes/ecs_notes.md` (Friflo benchmark survey + AGF-specific recommendations). Verdict: **don't rewrite to archetype ECS** — current `Map<ComponentName, Map<EntityId, ComponentData>>` is fine for AGF's scale and is agent-friendly. Instead, build the four sub-epics in order: (a) **ECS benchmark suite** (`ECS-B1..B5`) — harness + measure current Map query patterns + hierarchy resolve at 100/1k/10k entities + batch bucket collection, gate further changes on real numbers; (b) **LocalToWorld cache** (`M16-cache-a..e`) — TransformRevision + dirty subtree propagation + reused matrices + tests, sits under M21 renderer split; (c) **System-level command buffer** (`CMD-B1..B4`) — structural mutations queued during system iteration, applied between phases, recorded for diagnostics + replay; (d) **Explicit indexes** (`IDX-Parent`, `IDX-Networked`, `IDX-Assets`, `IDX-Pickable`) — add one when a real feature needs it, not earlier. Orient toward Friflo coverage + Unity-DOTS transform pipeline + Leopotam minimalism + AGF's schema-first authoring. Do NOT adopt: full archetype ECS, generic relations, event-driven gameplay. |
