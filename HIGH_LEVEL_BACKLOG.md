...
| `M15` Engine dev server | Active — investigation done, implementation pending | Sprint 30 produced `docs/research/engine-dev-server-investigation.md` settling the design: Vite plugin (`apply: "serve"`) + page-side bootstrap exposing `/__agf/*` HTTP + WS endpoints. Implementation sequenced as `M15-a` plugin scaffold → `M15-b` page bridge → `M15-c` pull endpoints → `M15-d` bug-report endpoint → `M15-e` recording → `M15-f` commands → `M15-g` SSE events → `M15-h` asset HMR trigger → `M15-i` optional engine connect CLI. |
