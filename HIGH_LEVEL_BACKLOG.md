...
Date: 2026-05-14 (Sprint 39 archived)
...
| `M21` Renderer → ECS systems | **Phase 1 ✅, Phase 2 ~80% shipped through S39** | `M21-investigate` shipped: see `docs/research/renderer-ecs-split-investigation.md`. **Phase 1 (structural split, 8 stories)**: thin `ThreeRenderAdapter` + five scheduler-registered systems + five renderer-internal components. Renderer-import-boundary preserved. **Phase 2 shipped through S39**: `M21-light-*` ✅, `M21-shadow-basic` / `-csm` / `-static` ✅, `M21-env-generated` ✅, `M21-mat-*` (5 shader kinds + textures) ✅, `M21-frame-timing` ✅, `M21-light-budgets` ✅, `M21-color` ✅ (config-driven tonemap, opt-in), `M21-context-loss` ✅, `RUNTIME-renderer-ready` ✅. Phase 2 remaining: `M21-shadow-algorithm` (PCSS / VSM stretch), `M21-mat-custom` (ShaderMaterial + onBeforeCompile), `M21-env-hdr` / `M21-env-cube`, `M21-cam-*`, `M21-post-pipeline` (Composer + Bloom / FXAA / SSAO / Outline), `M21-tsl-investigate`, `M21-webgpu-spike`. Out of scope at Phase 2 end: real-time GI, Forward+, animation system, scene editor. |
