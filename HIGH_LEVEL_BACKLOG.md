...
Date: 2026-05-14 (Sprint 41 archived)
...
| `M21` Renderer → ECS systems | **Phase 1 ✅, Phase 2 ~95% shipped through S41** | `M21-investigate` shipped: see `docs/research/renderer-ecs-split-investigation.md`. **Phase 1 (structural split, 8 stories)**: thin `ThreeRenderAdapter` + five scheduler-registered systems + five renderer-internal components. Renderer-import-boundary preserved. **Phase 2 shipped through S41**: `M21-light-*` ✅, `M21-shadow-basic` / `-csm` / `-static` / `-algorithm` (PCF + VSM + PCSS) ✅, `M21-env-generated` ✅, `M21-mat-*` (5 builtin shader kinds + textures + custom ShaderMaterial + external shader-file refs) ✅, `M21-frame-timing` ✅, `M21-light-budgets` ✅, `M21-color` ✅, `M21-context-loss` ✅, `M21-post-pipeline` ✅, `M21-cam-orbit` ✅, `RUNTIME-renderer-ready` ✅. Phase 2 remaining: `M21-shadow-pcss-csm` (extend PCSS into CSMShader), `M21-env-hdr` / `M21-env-cube`, `M21-cam-follow` / `M21-cam-cinematic`, `M21-tsl-investigate`, `M21-webgpu-spike`. Out of scope at Phase 2 end: real-time GI, Forward+, animation system, scene editor. |
