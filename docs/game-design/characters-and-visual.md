...
| **Slapstick death** | `DeathAnimationSystem` — fires `RagdollSpawnRequest` against the engine ragdoll module (`engine/physics/ragdoll/`, `GDP-2026-05-24-001`). Impulse vector derived from blast direction. Real Rapier joints handle the limb flail; arena floor + walls handle ground/clamp natively. Supersedes the S100 canned tween AND the S105+S108 procedural-spring path. | ~80 LOC project-side (template + impulse calc); engine module shoulders the simulation |
...
### 6.4 Procedural vocal synth — multi-syllable speech-like babble

**Decision (refined 2026-05-22): the bomber SPEAKS.** User explicitly
asked for Animal-Crossing-style "almost-words" rather than the
single-grunt direction the dev research artifact recommended.
Multi-syllable CV-babble (consonant + vowel pairs strung with
conversational cadence) is the production direction.

Design intent:
- Five emotional slots, each a SHORT UTTERANCE not a single envelope:
  `place-bomb` (3 syllables), `hit-survived` (2), `pickup` (3),
  `death` (4), `victory` (5). Each utterance 250–600 ms.
- Per-bomber **voice colour** = 10 seed-derived knobs (base pitch,
  F1/F2 formants, formant Q, vibrato rate + depth, noise mix, phrase
  pace, consonant style, vowel-drift amount). Same seed → same
  voice, always.
- Per-slot **phrase tables** (NOT seed-derived) define syllable
  count + pitch contour + vowel sequence per emotion. Same patches
  for every bomber — the colour layer carries identity, the patch
  layer carries emotion.
- Implementation: source-filter formant per vowel (sawtooth →
  parallel bandpass at F1/F2 → gain envelope) + consonant-burst
  transient per syllable (noise burst OR pitched click), scheduled
  in Web Audio with inter-syllable gaps. Zero audio assets.
- Listener test: blindfolded listener says "they're talking", not
  "they're beeping". Two bombers with different seeds are
  audibly distinguishable on the same slot.

Full design rationale + override list against the dev research
artifact lives in `docs/game-design/voice-synth-design.md`. Story:
`GDP-2026-05-22-008.story-proposal.json`.
