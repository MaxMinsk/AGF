...
describe("bomber spawn height lift (S173)", () => {
  ...
  it("S179 — entity authored on a stepped cell lifts to the cell height (heightmap-only)", () => {
    const heightmap = [
      [0, 1, 2]
    ];
    const scene: SceneInput = {
      ...
      entities: [
        {
          id: "grid.config",
          components: { Grid: { cellSize: 1, sizeX: 3, sizeZ: 1 } }
        },
        {
          id: "block.east",
          components: {
            Transform: { position: [2, 0.5, 0], rotation: [0, 0, 0], scale: [1, 1, 1] },
            GridPosition: { gx: 2, gz: 0 }
          }
        }
      ]
    };
    ...
    const commands = applyHeightmapCommands(scene);
    const lift = commands.find(
      (c) => c.kind === "component.set" && c.entityId === "block.east" && c.component === "Transform"
    );
    expect(lift).toBeDefined();
    const lifted = (lift as { data: { position: number[] } }).data.position;
    expect(lifted[1]).toBeCloseTo(0.5 + 2, 5);
  });
