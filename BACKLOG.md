...
1. **ASSET-textures-via-registry** — texture refs inside material manifests resolve through `AssetRegistry.get<TextureAsset>()` instead of `assetRegistry.urlFor()` + raw `TextureLoader`. 404s emit `AGF_RUNTIME_ASSET_LOAD_FAILED`; HMR can invalidate one texture without remounting the material. Includes a dedicated `createTextureLoader()` registered alongside the existing material / glb loaders. Material-bench texture references survive unchanged.
