...
- AGF-WEBGPU-CHUNK-SPLIT (engine S083) — investigate why vite.config.ts manualChunks rule for `three.webgpu` never produces a `three-webgpu-` chunk in production builds. The dynamic `import` in webgpu-module-loader.ts should split, but Vite/Rolldown pulls the WebGPU build (~180 KB gzipped of WebGPU/TSL/node-material code) straight into the main `three` chunk anyway. Result: every WebGL-only project (beacon-world, hello-3d, batch-bench, physics-bench, shadows-bench) ships WebGPU runtime it never uses. Fix → tighten the `three-` budget back from 560 → ~340 KB in scripts/check-bundle-size.mjs + engine/tools/doctor/project-doctor.ts. Hygiene CI was red on main from S75 through S81 because of this; S82 unblocked with a budget bump but the underlying bug remains.
