...
- **AGF-RENDER-DEBUG-FREECAM** — Agent-driven free-fly camera for debugging — POST /__agf/render/freecam _(implemented)_
  engine/render/three-render-adapter.ts grew a `freeCamCamera: PerspectiveCamera` override. `setFreeCam({ position, lookAt } | null)` creates / repositions the camera (added to the scene, FOV 60°, near 0.1, far 1000) or tears it down. `activeCamera()` now returns the freecam first when set, so render / resize / pickAtNdc all observe the override. Adapter.dispose() clears it. Forwarded through ThreeRenderer.{getFreeCam,setFreeCam}, app.{getRenderFreeCam, setRenderFreeCam}, window.__agf.{getRenderFreeCam, setRenderFreeCam}, and GET/POST /__agf/render/freecam on the dev bridge. POST body is validated by the exported pure `validateFreeCamBody` helper (accepts `{ off: true }` to clear OR `{ position, lookAt }` of finite-number tuples); invalid bodies return 400 AGF_BRIDGE_INVALID_FREECAM.
