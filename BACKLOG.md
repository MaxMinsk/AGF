...
- **KABOOM-TITLE-INPUT-PAUSE** — Player input freezes while GamePaused — fixes Space-to-start placing a bomb + movement-before-start _(implemented)_
...
- **KABOOM-AUDIO-PROCEDURAL-SFX** — WebAudio-synth SFX so the four events actually make sound _(implemented)_
...
- **AGF-POOL-WARMUP-PARTICLES** — Pre-warm the particle pool to its declared max so the first blast doesn't stall _(implemented)_
...
- **KABOOM-ROUND-TIMER** — Per-round time limit + auto-draw when nobody dies in time _(implemented)_
...
- **KABOOM-CONTROLS-HINT** — Bottom-center controls hint for the first 4 s of a fresh boot _(implemented)_
...
- **AGF-DOCS-AGENT-PROBE-CATALOGUE** — docs/agent-probes.md — one-page catalogue of every /__agf endpoint _(implemented)_
...
- **AGF-LOG-RENDERER-LIFT-WARN-SITES** — Lift the remaining renderer console.warn sites to runtime.diagnostics _(implemented)_
...
- **AGF-DOCTOR-RECOMMENDATION-HANDLE-LEAK** — engine doctor recommends action when rendererInspect.handleLeak > 0 _(implemented)_
