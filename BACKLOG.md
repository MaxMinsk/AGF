...
- **KABOOM-DAMPED-FOLLOW** — Damped follow camera system + `FollowTarget` / `CameraDamping` components _(implemented)_
...
- **KABOOM-HUD-RUNTIME** — 2D HUD runtime primitive (`engine/runtime/ui/`) _(implemented)_
...
- **KABOOM-MINIMAP-WIDGET** — Generic minimap widget on top of the HUD runtime _(implemented)_
...
- **KABOOM-GRID-POSITION** — `GridPosition` + `GridOccupant` components + `Grid` config schema _(implemented)_
