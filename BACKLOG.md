...
- **QA-DOCS-CLAUDE-MD** — Update CLAUDE.md + AGENTS.md with the QA-workflow section _(implemented)_
  CLAUDE.md gets a new §QA-Workflow section right after Working Mode: file-ownership table (dev vs QA paths), per-story sequence on the dev terminal (rebase off main → claim → implement → acceptance: line in verification[] → done → render → commit), sprint planning (qa:promote + engine doctor's QA inbox line), unchanged sprint close. Working Mode gains one new bullet: rebase off main at every story start. AGENTS.md gains a short paragraph right after the CLAUDE.md pointer that tells the QA agent to read docs/qa/agent.md instead.
