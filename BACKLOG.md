...
- **GAME-DESIGN-AGENT-ROLE-DOC** — Seed docs/game-design/agent.md — third-role workflow + GDD location _(implemented)_
  First concrete artifact under the GAME-DESIGN-AGENT epic. docs/game-design/agent.md mirrors docs/qa/agent.md: defines the role (game-designer / product-owner), the file-ownership table (you write the GDD + proposed-stories; you don't touch engine/, tests, sprint JSONs, or QA tickets), the proposed-story ticket shape (sketch — schema lands in a follow-up sprint), the per-sprint workflow loop, three launch options (kickoff prompt / subagent / standalone). docs/game-design/gdd.md is a stub with section headers (Overview, Mechanics, Progression, World, Characters, UX, Open questions) — content fills in over time. Updated backlog/epics/GAME-DESIGN-AGENT.epic.json notes to call out this seed + lists the remaining work.
- **DOC-AGENT-DEBUG-RECIPES** — docs/agent-debug-recipes.md — cookbook for common probe flows _(implemented)_
  docs/agent-debug-recipes.md ships with 10 recipes, each titled, each with a copy-paste curl one-liner + jq filter where useful: (1) grab browser logs first, (2) snapshot diff to inspect last-tick changes, (3) focused component lookup live + historical, (4) detect one-shot events via diff, (5) god-view freecam for off-screen debugging, (6) render-debug-mode for wireframe / normals / uv, (7) capture a recording for regression, (8) time-scale slowdown + paired with diff, (9) audio master volume mute / restore, (10) renderer pool + handle leak hunt. Every recipe cross-links into docs/agent-probes.md for the reference. CLAUDE.md §Read First gains a one-line pointer to the new file.
