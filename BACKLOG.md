...
- **KABOOM-KICK-POWER-UP** — Bumping into your own bomb kicks it one cell forward (GDP-003) _(pending)_
  Promoted from GDP-2026-05-20-003. When a bomber walks INTO a cell occupied by one of their own bombs, the bomb slides one cell in the same direction (instead of blocking movement). If the next cell is hard-blocked / has another bomb / has a wall, the kick fails silently and the bomber stays put (movement-blocked as today). Bombs already in transit (currentLerp > 0) cannot be kicked. Project-local: lives in examples/kaboom-crew/src/systems/bomb-kick-system.ts. Reuses GridOccupancy for collision checks.
- **KABOOM-SLAPSTICK-DEATH** — Death launch-up + spin-out tween before despawn (GDP-004) _(pending)_
...
- **AGF-DOCTOR-REJECTED-INBOX** — engine doctor reports the rejected-QA-ticket count too _(pending)_
