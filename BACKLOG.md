...
10. **`M24-interpolation`** — interpolate dynamic body transforms between fixed steps before rendering. Today Rapier runs at 1/60 fixed; on 120 Hz displays two rAF callbacks share one physics position, giving a 60 Hz visual pulse even at 120 fps. Plan: store `prevPos`/`prevRot` at the start of each fixed step + `currPos`/`currRot` at the end, expose `alpha = accumulator / fixedDt` to `MeshTransformSyncSystem`, lerp/slerp before writing to the renderer. Refresh-rate-independent visuals without changing the simulation cadence. Pairs naturally with `M21-frame-timing` since both surface as agent-visible signals.
