...
- **QA-FIX-NOTES-GITIGNORE** — Unblock notes/qa/** for artefact capture (close QA-2026-05-20-001) _(pending)_
  QA filed QA-2026-05-20-001 (now BUG-NOTES-QA-IS-GITIGNORED-001 in this sprint after promote): `Notes/` in .gitignore matches `notes/` on macOS's case-insensitive filesystem, so QA can't commit screenshots/repros under notes/qa/. Two options: (a) carve out a `!notes/qa/` exception in .gitignore; (b) move QA artifacts to a different tracked dir (e.g. `docs/qa/artifacts/`). Decision in this story; pick (a) because docs/qa/agent.md + the schema already reference notes/qa/ paths — the exception keeps backward-compat for any existing screenshots an agent might author.
