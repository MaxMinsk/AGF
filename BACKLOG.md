...
- **KABOOM-MIGRATE-PREFABS** — Kaboom Crew player.prefab + bot.prefab use procedural:procbomber tree _(implemented)_
...
- **KABOOM-BOMBER-ANIMATION-PROD** — Drive bomber animation kind from gameplay state (idle / walk / reach / death) _(implemented)_
...
- **KABOOM-REACH-IK-PLACE-BOMB** — Reach IK animation triggered by PlaceBombRequest _(implemented)_
...
- **AGF-MESH-LIFECYCLE-RESTORE-HANDLE-COMPONENT** — MeshLifecycleSystem restores RENDER_MESH_HANDLE after scene.load _(implemented)_
  Engine bug surfaced during the kaboom-crew migration's round-restart path. scene.load wipes every entity component but the renderer's MeshHandleRegistry (renderer-level cache) keeps its per-entity-id handle table. When the new scene re-creates an entity with the same id, the registry returns the existing handle, so MeshLifecycleSystem's acquire branch was skipped — leaving the new entity without the RENDER_MESH_HANDLE component that MeshTransformSyncSystem + MaterialBindingSystem query for. Result: the mesh's Three.js transform stayed frozen at its pre-scene-load value. Fix: always re-write RENDER_MESH_HANDLE for every renderable that has a handle, whether the handle is fresh or cached.
