...
## Current Sprint: Sprint 36 — physics integration + cache polish + batched-mesh primitives + Beacon physics adoption (DONE — archive merging)

Sprint 36 integrated the Rapier adapter shipped in Sprint 35 into the runtime, finished the cache work, laid down `BatchedMesh` adapter primitives, fixed a renderer M17 double-acquire bug surfaced by `batch-bench`, and turned on physics declarations in Beacon. Eight stories landed; `M21-shadow-csm` and `M17-batched-mesh-system` carried forward to Sprint 37.
...
- `M17-batched-mesh` ✅ Implemented (adapter primitives only). `acquireBatchedBucket` / `releaseBatchedBucket` / `addBatchedGeometry` / `addBatchedInstance` / `removeBatchedInstance` / `setBatchedInstanceTransform` / `setBatchedInstanceGeometry` / `batchedBucketLiveCount`. Internal `batchedBuckets` map + `nextBatchedBucketHandle`. `rendererInfo()` exposes `batchedBuckets` / `batchedBucketInstances`. System-layer wiring (BatchingSystem extension for `Batchable.path = "batched"`) deferred to Sprint 37.
- `examples/batch-bench/` ✅ Implemented. Perf-only project — camera + ambient + sun + ground; no project-specific systems. Bootstrap seeds a default 20×20 grid (400) of batchable cubes in 4 colours via `attachUi` → `runtime.applyCommands`. `?seed=N` URL param overrides (clamped 1..4096); `?seed=0` keeps the empty baseline. Acts as a live regression target — open the page and `__agf.rendererInfo()` reports `drawCalls: 5, buckets: 4, bucketInstances: 400, handleLeak: 0`.
- `M17-renderer-respect-batchable` ✅ Implemented. Surfaced by `batch-bench`: the standalone renderer fallback (`refreshMeshes`) acquired a per-entity Mesh handle for every `MeshRenderer` regardless of `Batchable`, doubling draw calls and leaking handles when `MeshLifecycleSystem` had already filtered the entity. `engine/render/three-renderer.ts` now filters `Batchable` out of `refreshMeshes` the same way the system does. On the 400-box bench grid: `drawCalls 405 → 5`, `meshes 401 → 1`, `handleLeak 400 → 0`.
- `beacon-physics-adoption` ✅ Implemented. `examples/beacon-world/project.json` enables `physics: { enabled: true, gravity, fixedDt }`. Scene declares: `ground` (fixed body + box collider), `player.drone` (kinematicPosition body + capsule, lockRotations), `beacons` (fixed body + capsule sensor), `cores` (fixed body + sphere sensor), `hazards` (fixed body + sphere sensor sized to maxRadius). Gameplay systems (`pickup-system`, `hazard-system`) still drive via Transform distance — sensor-event wiring + character-controller-driven movement are Sprint 37 candidates. Bridge probe: 8 bodies / 6 sensors / zero diagnostics; `beacon-world-gameplay.spec` green.

### Carried to Sprint 37

- **M21-shadow-csm** — outdoor Cascade Shadow Maps via `three/addons/csm/CSM.js`. High-touch: needs an adapter-side CSM instance bound to the active camera, per-frame `csm.update()`, and `csm.setupMaterial(material)` called on every material the renderer manages (acquire / patch / manifest paths). Schema lands on `project.json#renderer.shadows.csm`. Conflict with the ECS-owned sun's shadow needs an opt-out path.
- **M17-batched-mesh-system** — wire the BatchedMesh adapter primitives behind a `Batchable.path?: "instanced" | "batched"` selector in `BatchingSystem`. Bucketing for "batched" keys by `material + shadow + group` (mesh varies); useful when many distinct geometries share one material. Add a `batch-bench` scenario that pushes 4 different primitive meshes through one batched bucket and confirms drawCalls stay flat.
- **beacon-physics-character** — switch `player.drone` from `PlayerControlled` Transform writes to a `CharacterController3D` + a new `CharacterMovementSystem` that consumes input + queries the controller for collision-resolved motion. Requires a project flag so static scenes (Hello-3D) keep the old path.
- **beacon-physics-sensor-wiring** — replace `pickup-system` / `hazard-system` proximity loops with `OverlappingTriggers3D` reads. Gameplay events stay identical; the radius checks become sensor-collider acquisitions.
...
- `M24-adapter` ✅ Implemented. `engine/physics/rapier/rapier-adapter.ts` — lazy-init wrapper. Dynamic `await import('@dimforge/rapier3d-compat')` inside `createRapierAdapter()`, plus a `createAdapterFromModule(RAPIER, options)` factory for test/spike code that already init'd RAPIER. Internal handle maps: `bodies` / `colliders` + reverse `colliderBody` + `bodyColliders` so `releaseBody` purges its child colliders (mirrors Rapier's auto-removal). API: `acquireBody / releaseBody / acquireCollider / releaseCollider / setBodyTransform / getBodyTranslation / getBodyRotation / step / setGravity / info / dispose`. All four v0 collider kinds (box / sphere / capsule / cylinder) wired with per-kind required fields. `spikes/physics-rapier-v0/adapter-spike.ts` exercises the full lifecycle (cube + ball + capsule fall + release → bodies=0, colliders=0).
