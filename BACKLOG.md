...
8. **RUNTIME-progressive-loading** — declarative critical-asset gate via `project.json#render.criticalAssets: string[]`. `startRuntime` keeps the existing placeholder-then-swap path for every other asset; `rendererReady` only resolves once each ref in the list has reached `applied` or `failed` on its `AppliedMaterialRef` / `AppliedGeometryRef`. Helper `criticalAssetsReady(world, refs)` is exported for tests + tooling. Status: Implemented.
9. **RUNTIME-idle-rendering** — `World.mutationCounter()` exposes a monotonic counter bumped on every `addEntity` / `removeEntity` / `setComponent` / `removeComponent`. `startRuntime` honours `project.json#render.idleMode: "always" | "on-demand"`: in `on-demand` it skips `renderer.render()` for frames where the counter hasn't moved. First frame after boot + every window resize + frames after any system mutation always render. Tests: `tests/unit/world-mutation-counter.test.ts`. Status: Implemented.
