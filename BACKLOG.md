...
- **SHADOWS-bench-perf-deepdive** — after S51 dropped `shadows.autoUpdate: false`, shadows-bench renders correctly (cars + swaying trees now move their shadows) but FPS dropped vs the baked-shadow baseline. The whole scene (305 entities, 3 CSM cascades, PCSS, FXAA) re-shadow-maps every frame. Need a full breakdown of where the budget goes + which knobs are worth turning before flipping `autoUpdate` back on, hybridising static/dynamic shadow casters, or adopting BatchedMesh.perObjectFrustumCulled. The deliverable is a measured report at `docs/research/m21-shadows-bench-perf.md` followed by ranked follow-up stories — not a fix sprint. Concrete hypotheses + probes to land in the report:
  - **Shadow pass cost per cascade.** Measure `renderer.info.render.calls` × `triangles` per cascade vs main pass via `WebGLRenderer.info` snapshots before/after each `composer.render()`. Hypothesis: each cascade re-runs the whole vertex shader for all 305 entities × 3 cascades = ~915 draw-pass equivalents, dominating frame time. Probe: temporarily set `cascades: 1` and `cascades: 2`, record FPS delta — confirms whether the cascade count is the multiplier.
  - **PCSS fragment cost.** PCSS replaces `getShadow` with a multi-tap blocker search + penumbra estimator (per-pixel, per-light). On 1024² shadow map with a 1080p viewport that's millions of fragment-shader taps per frame. Probe: flip `algorithm: "pcf"` and compare FPS at identical scene + autoUpdate=true. Hypothesis: PCSS costs more than the autoUpdate switch.
  - **CSM material rebuild on dynamic geometry.** `csm.setupMaterial(material)` is called once per material at registration, but the moving cars + swaying tree LTW changes might be triggering shader-program churn (`programs` counter rising). Probe: log `renderer.info.programs.length` every 60 frames; should stay flat. If it grows, batching's bucket lifecycle is leaking programs.
  - **`shadowMap.needsUpdate` overhead vs autoUpdate.** With `autoUpdate: true` the renderer recomputes every cascade every frame regardless of whether any caster moved. For shadows-bench only ~12 entities move (6 cars + 6 tree roots tween) — the other ~290 are static. Probe: keep `autoUpdate: false` + call `renderer.shadowMap.needsUpdate = true` once per frame from a system; if FPS recovers, the per-cascade pass itself isn't the bottleneck, the *redundant work for static casters* is. Then the real fix is `BatchedMesh.perObjectFrustumCulled` + per-caster dirty tracking.
  - **InstancedMesh shadow-pass cost.** Each InstancedMesh draws all N instances per shadow pass even if only a few are visible. Probe: read `renderer.info.render.triangles` with `cascades: 1`, compare InstancedMesh path vs single-Mesh path (toggle `batching.auto`). If triangles per cascade ≫ visible triangle count, per-instance frustum culling (needs BatchedMesh + BVH, see M17-batched-mesh-primary) is the win.
  - **`shadowNormalBias = 0.12` is high.** Was tuned at `shadowMapSize: 2048` and never re-checked after the drop to 1024. Probe: try 0.04–0.08 with PCF + 1024 — if peter-pan returns, document the trade; if not, lower bias means cleaner contacts.
  - **Maybe FXAA on a dynamic scene with PCSS dithering reintroduces shimmer.** Probe: disable FXAA, compare visual quality + FPS. Hypothesis: FXAA is essentially free here but worth confirming.
  - **Dev overlay overhead.** When the dev overlay is open the snapshot endpoint serializes the whole world every 250 ms. Probe: close the overlay and measure FPS via `__agf.rendererInfo()` from a one-shot console call — confirms whether the readout itself is biased.
  - Acceptance: `docs/research/m21-shadows-bench-perf.md` containing (1) baseline FPS + frame-time + per-pass triangle counts at the current settings, (2) per-probe deltas above with raw numbers, (3) a ranked list of follow-up stories (e.g. `M21-static-shadow-bake` / `M17-batched-mesh-primary` / `M21-shadow-cascade-1-for-bench` / `M21-pcss-cheaper`) sized by FPS gain × implementation effort. Probes run on the user's machine when free, not in CI (software WebGL caps at ~5 fps).
