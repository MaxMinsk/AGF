...
- **KABOOM-WIN-CHIME** — Short procedural chime on match win/lost/draw _(implemented)_
  AudioEventKind grows three new values — match-won / match-lost / match-draw — and audio-binding-system tracks the RoundState.matchPhase singleton each fixed step, firing the matching event exactly once per transition out of 'in-progress'. audio-fx.ts adds matching procedural chords: C major triad (won), A minor descent (lost), neutral perfect-fifth (draw). Inherits the existing volume dial through masterGain.
- **KABOOM-BOT-DANGER-AVOID** — Bot prefers safe neighbour cells over dangerous ones (and live BlastTiles count) _(implemented)_
  Today's BotAISystem treats BlastTile cells as walkable in the wander path — the bot regularly steps into a live blast it just placed. Two fixes: (1) buildDangerMap now also marks every cell covered by a live BlastTile, not just the projected blast rays of un-exploded bombs. (2) decideDirection always prefers neighbours not in the danger map (was: only the flee path filtered), and the directional bias only kicks in when the chosen direction is in the safe pool. Falls back to ANY neighbour when every adjacent cell is dangerous so the bot still moves when boxed in.
- **AGF-LOG-LIFECYCLE-SCHEDULER** — scheduler.register emits an info-level AGF_SCHEDULER_SYSTEM_REGISTERED diagnostic _(implemented)_
  Engine lifecycle traces follow-up from S082 — the SystemScheduler already emitted AGF_SCHEDULER_SYSTEM_REGISTERED / _DEREGISTERED info events when a DiagnosticsBus was supplied (with unit-test coverage under tests/unit/lifecycle-traces.test.ts), but the production wiring in src/app.ts constructed the scheduler without the bus, so the events never surfaced in /__agf/diagnostics. This story finishes the loop by passing the shared diagnostics bus into the scheduler constructor.
