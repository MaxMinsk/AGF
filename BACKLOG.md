...
- **AGF-PROBE-DIAGNOSTICS-SINCE** — GET /__agf/diagnostics?since=<timestamp> filters by timestamp _(implemented)_
  engine/runtime/diagnostics/diagnostics-bus.ts grew `snapshotSince(thresholdSeconds)` and an exported pure `filterDiagnosticsSince(diagnostics, threshold)` helper (returns strict suffix where emittedAtSeconds > threshold; non-finite threshold returns all). Plumbed through src/app.ts.diagnosticsSince → window.__agf.diagnosticsSince → page-bridge `diagnostics-since` RPC. Dev-bridge GET /diagnostics handles `?since=<ISO>`: parses via Date.parse, returns 400 AGF_BRIDGE_INVALID_DIAGNOSTICS_SINCE on non-ISO, otherwise forwards `thresholdSeconds = sinceMs / 1000` to the page. Without `?since`, the existing /diagnostics route still returns the full ring.
- **KABOOM-CONTROLS-HINT-FADE** — Controls-hint banner fades out the same way as the title screen _(implemented)_
  Mirrors S096 KABOOM-TITLE-SCREEN-FADE. controlsHintSpec.render now reads an `opacity` field (default 0.85, the pre-S097 baseline). When the hint-window closes (4 s in OR banner needs the slot), the update loop kicks `controlsHintFadeStartMs`, samples `fadeOutOpacityCurve(elapsed, 350)` scaled by 0.85 each frame, calls `hud.update(CONTROLS_HINT_ID, { opacity })`, and finally `hud.remove`s the widget when opacity reaches 0. Uses the same pure helper from S096 — no new code path.
- **DOC-PROPOSED-STORY-RECIPE** — docs/agent-debug-recipes.md — game-design proposed-story workflow recipe _(implemented)_
  Recipe #11 ships in docs/agent-debug-recipes.md: the 5-step game-design agent loop with copy-paste npm commands. (1) re-read gdd.md + last archive entry, (2) `npm run propose:new -- "<title>" --kind feature --priority should`, (3) replace TODO: lines, (4) `npm run backlog:check` to validate, (5) wait for the dev terminal's `npm run propose:promote -- --into S<N>`. Cross-links to docs/game-design/agent.md and docs/game-design/gdd.md so the recipe is the entry point for someone discovering the workflow.
